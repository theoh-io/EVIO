...
class Model3(BaseModel):
    def __init__(self, opt):
        super(Model3, self).__init__(opt)
        self._name = 'Model3'
    ...
    def _init_set_input_params(self):
        ...
        self._S1, self._S2 = self._opt[self._dataset_type]["image_size"]               # image size
        self._Ci = self._opt[self._dataset_type]["img_nc"]                  # num channels image
        #other Modalities
        self._E=self._opt["dataset"]["event_dim"]
        self._I=self._opt["dataset"]["imu_dim"]
        self._Ct = self._opt[self._dataset_type]["target_nc"]     # num channels target
    ...
    def _init_prefetch_inputs(self):
        self._input_img = torch.zeros([self._B, self._Ci, self._S1, self._S2]).to(self._device_master)
        self._input_ev=torch.zeros([self._B,self._E])
        self._input_imu=torch.zeros([self._B,self._I])
        self._input_target = torch.zeros([self._B,self._Ct]).to(self._device_master)

    def set_input(self, input):
        # copy values
        self._input_img.copy_(input['img'])
        self._input_ev.copy_(input['event'])
        self._input_imu.copy_(input['imu'])
        self._input_target.copy_(input['target'])

        # move to gpu
        self._input_img = self._input_img.to(self._device_master)
        self._input_ev = self._input_ev.to(self._device_master)
        self._input_imu = self._input_imu.to(self._device_master)
    ...
    def forward(self, keep_data_for_visuals=False, estimate_loss=True):
        # generate img
        estim = self._estimate(self._input_img, self._input_ev, self._input_imu)
    ...
    def _estimate(self, img, ev, imu):
        return self._reg.forward(img, ev, imu)
